...
class RealImageFolderDataset(torch.utils.data.Dataset):
    def __init__(self, directory, train=True):
        ...
        # ])
        images = sorted([
            os.path.join(directory, f,"rendered_scene_256.png")
            for f in os.listdir(directory)
...
def main(argv):
    ...
    # Create the real datasets
    train_real = RealImageFolderDataset(
        args.path_to_real_renderings,
        True
    )
    test_real = RealImageFolderDataset(
        args.path_to_real_renderings,
        False
    )
            ...
            if (e + 1) % 5 == 0:
                with torch.no_grad():
                    ...
                    for i, (x, y) in enumerate(test_dataloader):
                        ...
                        acc = (torch.abs(y-y_hat) < 0.5).float().mean()
                        acc1 = (torch.abs(y-y_hat)[y==1] < 0.5).float().mean()
                        acc0 = (torch.abs(y-y_hat)[y==0] < 0.5).float().mean()
                        loss_meter += loss
                        acc_meter += acc

...
class ThreedFront(BaseDataset):
    ...
    def __init__(self, scenes, bounds=None):
        ...
        # scenes_train_dataset = self.from_dataset_directory(
        ...
        #     filter_fn=filter_function(cfg["dataset"], cfg["train"].get("splits", ["train", "val"]), cfg["dataset"]["without_lamps"])
        # )

        # assert isinstance(self.scenes[0], Room)   #TODO
        ...
        if bounds is not None:
            ...
            if "objfeats_pc_ulip" in bounds.keys():
                self._objfeats_pc_ulip = bounds["objfeats_pc_ulip"]
            elif "objfeats_pc_ulip" in bounds.keys():
                self._objfeats_pc_ulip = bounds["objfeats_pc_ulip"]
    ...
    def _compute_bounds(self):
        ...
        _objfeat_pc_ulip_std = np.array([1])
        _objfeat_pc_ulip_min = np.array([10000000000])
        _objfeat_pc_ulip_max = np.array([-10000000000])
        all_objfeats = []
        all_objfeats_32 = []
        all_objfeats_pc_ulip = []
        for s in self.scenes:
            for f in s.bboxes:
                if np.any(f.size > 5):
                ...
                # all_objfeats_pc_ulip.append(f.raw_model_norm_pc_lat_ulip())


        # all_objfeats = np.stack(all_objfeats, axis=0)
        all_objfeats_32 = np.stack(all_objfeats_32, axis=0)
        # all_objfeats_pc_ulip = np.stack(all_objfeats_pc_ulip, axis=0)
        ...
        # _objfeat_pc_ulip_std, _objfeat_pc_ulip_min, _objfeat_pc_ulip_max = np.array([all_objfeats_pc_ulip.flatten().std()]), np.array([all_objfeats_pc_ulip.min()]), np.array([all_objfeats_pc_ulip.max()])
        ...
        # self._objfeats_pc_ulip = ( _objfeat_pc_ulip_std, _objfeat_pc_ulip_min, _objfeat_pc_ulip_max )
    ...
    def bounds(self):
        return {
            ...
            # "objfeats_pc_ulip": self._objfeats_pc_ulip,
        }
    ...
    @property
    def objfeats_pc_ulip(self):
        if self._objfeats_pc_ulip is None:
            self._compute_bounds()
        return self._objfeats_pc_ulip
    ...
    def class_labels(self):
        if 'bedroom' in self.room_types:
            # #bedroom
            return ["armchair", "bookshelf", "cabinet", "ceiling_lamp", \
                    "chair", "children_cabinet", "coffee_table", "desk", \
                    "double_bed", "dressing_chair", "dressing_table", \
                    "kids_bed", "nightstand", "pendant_lamp", "shelf", \
                    "single_bed", "sofa", "stool", "table", "tv_stand", \
                    "wardrobe", "empty"]
        else:
            #dining room
            return  ["armchair", "bookshelf", "cabinet", "ceiling_lamp", \
                    ...
                    "round_end_table", "shelf", "stool", "tv_stand", "wardrobe", \
                    "wine_cabinet", "empty"]
        #TODO change to cache mode 
        #TODO can compute class_labels in the dataset
    ...
    def max_length(self):
        if self._max_length is None:
            ...
            for _room_type in _room_types:
                if 'bed' in _room_type:
                    self._max_length = max(self._max_length,12)
                elif 'living' in _room_type:
                    self._max_length = max(self._max_length,21)
                elif 'dining' in _room_type:
                    self._max_length = max(self._max_length,21)
                elif 'library' in _room_type:
                    self._max_length = max(self._max_length,11)
...
class CachedThreedFront(ThreedFront):
    def __init__(self, base_dir, config, scene_ids):
        ...
        self._parse_train_stats(train_stats)
    ...
    def __getitem__(self, i):
        ...
        return CachedRoom(
            ...
            objfeats_pc_ulip=D["objfeats_pc_ulip"] if "objfeats_pc_ulip" in D.keys() else None,
    ...
    def get_room_params(self, i):
        ...
        if "objfeats_pc_ulip" in D.keys():
            data_dict[ "objfeats_pc_ulip" ] = D["objfeats_pc_ulip"]
    ...
    def _parse_train_stats(self, train_stats):
        ...
        if "bounds_objfeats_pc_ulip" in train_stats.keys():
            self._objfeats_pc_ulip = train_stats["bounds_objfeats_pc_ulip"]
            print("bounds_objfeats_pc_ulip of dataset:", self._objfeats_pc_ulip)
            self._objfeats_pc_ulip = ( np.array([self._objfeats_pc_ulip[0]]), np.array([self._objfeats_pc_ulip[1]]), np.array([self._objfeats_pc_ulip[2]]) )
        else:
            self._objfeats_pc_ulip = ( np.array([1]), np.array([-1]), np.array([1]) )

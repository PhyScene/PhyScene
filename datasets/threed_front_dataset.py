...
class DatasetDecoratorBase(Dataset):
    ...
    def post_process_cuda(self, s):
        return self._dataset.post_process_cuda(s)
...
class BoxOrderedDataset(DatasetDecoratorBase):
    def __init__(self, dataset, box_ordering=None):
        ...
        for i in range(len(self._dataset)):
            self.bbox_dict[i] = self._get_boxes_(i)

    @lru_cache(maxsize=16)
    def _get_boxes(self, scene_idx):
        return self.bbox_dict[scene_idx]

    @lru_cache(maxsize=16)
    def _get_boxes_(self, scene_idx):
...
class ObjFeatPcdULIP512Encoder(DataEncoder):
    @property
    def property_type(self):
        return "objfeats_pc_ulip"
    ...
    def __getitem__(self, idx):
        ...
        # Get the rotation matrix for the current scene
        L = len(boxes)  # sequence length
        C = len(boxes[0].raw_model_norm_pc_lat_ulip())
        latents = np.zeros((L, C), dtype=np.float32)
        for i, bs in enumerate(boxes):
            latents[i, :] = bs.raw_model_norm_pc_lat_ulip()
        return latents
...
class RotationAugmentation(DatasetDecoratorBase):
    ...
    def __init__(self, dataset, min_rad=0, max_rad=np.pi*2, fixed=False):
        super().__init__(dataset)
...
class FlipAugmentation(DatasetDecoratorBase):
    def __init__(self, dataset):
        super().__init__(dataset)
    ...
    def __getitem__(self, idx):
        ...
        if self.random_flip:
            for k, v in sample_params.items():
                if k == "translations":
                    sample_params[k][:,0] = -sample_params[k][:,0]
                elif k == "angles":
                    sample_params[k] = -sample_params[k]
                elif k == "room_layout":
                    sample_params[k] = np.array(list(sample_params[k][:,:,::-1])) #torch.Tensor(list(sample_params[k].cpu().numpy()[:,:,::-1])).cuda()
    ...
    def post_process_cuda(self, s):
        ...
        for k, v in s.items():
            if k == "room_layout" or k == "class_labels" or k == "relations" or k == "description" or k == "desc_emb":
                ...
                sample_params[k] = Scale.descale(
                    v, torch.Tensor(bounds[k][0]).cuda(), torch.Tensor(bounds[k][1]).cuda()
                )
        return super().post_process_cuda(sample_params)
...
class Scale_CosinAngle(DatasetDecoratorBase):
    ...
    def __getitem__(self, idx):
        ...
        for k, v in sample_params.items():
            ...
            elif k == "objfeats" or k == "objfeats_32" or "objfeats" in k:
                continue
...
class Scale_CosinAngle_ObjfeatsNorm(DatasetDecoratorBase):
    ...
    def __getitem__(self, idx):
        ...
        bounds = self.bounds
        # bounds={'translations': (np.array([-5.67291869,  0.0375    , -5.71640158]), 
        #                          np.array([5.09667922, 3.35774051, 5.40485   ])), 
        #         'sizes': (np.array([0.03999   , 0.02000002, 0.02799703]), 
        #                   np.array([2.38027  , 1.770065 , 1.4137885])),
        #          'angles': (np.array([-3.14159265]), np.array([3.14159265])), 
        #          'objfeats_32': (np.array([0.96590555]), np.array([-4.5995083]), np.array([4.2565713]))}
        sample_params = self._dataset[idx]
        for k, v in sample_params.items():
            if k == "angles":
                # [cos, sin]
                ...
                #clip and normalize value into [0,1] according to bounds 
                sample_params[k] = Scale.scale(
                    v, bounds[k][0], bounds[k][1]
                )
    ...
    def post_process(self, s):
        ...
        sample_params = {}
        for k, v in s.items():
            if k == "room_layout" or k == "class_labels" or k == "relations" or k == "description" or k == "desc_emb" or k=="objectness":
                sample_params[k] = v
    ...
    def post_process_cuda(self, s):
        ...
        for k, v in s.items():
            if k == "room_layout" or k == "class_labels" or k == "relations" or k == "description" or k == "desc_emb":
                ...
                # theta = arctan sin/cos y/x
                sample_params[k] = torch.arctan2(v[:, :, 1:2], v[:, :, 0:1])
...
#random choose K objects from L 
...
class Diffusion(DatasetDecoratorBase):
    #todo permutation empty node
    def __getitem__(self, idx):
    
        
        sample_params = self._dataset[idx]
         # max number of bounding box in this scene (e.g. bedroom)
        ...
        for k, v in sample_params.items():
            if k == "room_layout" or k == "length":
                ...
                # new_class_labels = np.concatenate([class_labels[:, :-1], class_labels[:, -1:]], axis=-1) #hstack
                L, C = class_labels.shape
                assert max_length - L >= 0
                # Pad the end(empty) label in the end of each sequence, and convert the class labels to -1, 1
                end_label = np.eye(C)[-1]
                sample_params_target[k] = np.vstack([
                    class_labels, np.tile(end_label[None, :], [max_length - L, 1])
                ...
                # sample_params_target[k] = np.vstack([p, np.tile(np.ones(C)[None, :], [max_length - L, 1])]).astype(np.float32)
...
def dataset_encoding_factory(
    ...
    if "cached" in name:
        if "objfeats" in name:
            ...
            elif "pc_ulip" in name:
                dataset_collection = OrderedDataset(
                    CachedDatasetCollection(dataset),
                    ["class_labels", "translations", "sizes", "angles", "objfeats_pc_ulip"],
                    box_ordering=box_ordering
                )
                print("use pc_ulip as objfeats")
            else:
                dataset_collection = OrderedDataset(
                    CachedDatasetCollection(dataset),
    ...
    if augmentations!= None and len(augmentations)>0:
        for aug_type in augmentations:
            if aug_type == "rotations":
                ...
                print("Applying flip augmentations")
                dataset_collection = FlipAugmentation(dataset_collection)
    ...
    if "cosin_angle" in name and "objfeatsnorm" in name:
        print('use cosin_angles instead of original angles, AND use normalized objfeats')
        dataset_collection = Scale_CosinAngle_ObjfeatsNorm(dataset_collection)
    elif "cosin_angle" in name:
        print('use cosin_angles instead of original angles')
    ...
    if "objfeats" in name:
        if "lat32" in name:
            ...
            permute_keys.append("objfeats_pc_ulip")
        else:

...
def parse_threed_front_scenes(
    ...
    PATH_TO_SCENES = os.getenv("PATH_TO_SCENES")
    if os.path.exists(PATH_TO_SCENES):
        ...
        scenes = sum(scenes, [])
        path_dir = "/".join(PATH_TO_SCENES.split("/")[:-1])
        if not os.path.exists(path_dir):
            os.mkdir(path_dir)
        save_path = PATH_TO_SCENES
        pickle.dump(scenes, open(save_path, "wb")) #/tmp/threed_front.pkl
